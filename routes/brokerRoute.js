 ...
 * Description: HTTP requests to register and login a broker, handle stocks, execute trades, and update stocks (last three are not yet implemented).
 */
...
router.post("/register", async (req, res) => {
    try {
        ...
        res.status(500).json({
            message: "Error registering broker.",
            error: error.message,
        });
    }
});
...
router.post("/login", async (req, res) => {
    try {
        ...
        const token = jwt.sign({ brokerId: broker._id }, "BackEndBroker@123", {
            expiresIn: "1h",
        });
